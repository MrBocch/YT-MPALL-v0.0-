...
def download_video(link, format_option):
    try:
        yt = YouTube(link)
        print(f"Descargando {yt.title}")
        if format_option == '1':            yt.streams.filter(file_extension='mp4').first().download(output_path="downloads")
        elif format_option == '2':           yt.streams.filter(file_extension='avi').first().download(output_path="downloads")
        elif format_option == '3':            yt.streams.filter(file_extension='mkv').first().download(output_path="downloads")
        else:
            print("Opción no válida.")
    except Exception as e:
        print(f"Error descargando {link}: {e}")
